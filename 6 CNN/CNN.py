...
class CNN():
    ...
    def fit(self,X,Y,draw = False):
        ...
        tmpX = X.reshape((X.shape[0]//60,60,1,X.shape[1],X.shape[2]))
        tmpY = Y.reshape((X.shape[0]//60,60,10))

        for i in range(self.epochs):
            for idx in range(X.shape[0]//60):
                outX = self.net(tmpX[idx])
                trainer.zero_grad()
                loss = self.loss_func(outX, tmpY[idx])
                loss.backward()
                trainer.step()
            if draw:
                plt_x.append(i)
...
if __name__ == '__main__':
    ...
    cnn = CNN(epochs=10,loss_func=nn.CrossEntropyLoss())
